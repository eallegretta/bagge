(function($) {
	...
	$.fn.pagedBulletedList = function(options) {
		...
		function createTable(ul) {
			var table = $("<table />")
					...
					.width(config.width);
			$("li", ul).each(function() {
				var tr = $("<tr />").append($("<td />").append($(this).html()));
				table.append(tr);
			});
			return table;
		}
		...
		this.each(function() {
			...
			table.prepend("<tr><th>" + config.title + "</th></tr>");
		});
